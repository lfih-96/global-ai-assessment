...
const source = readFileSync(new URL('../public/effects.js', import.meta.url), 'utf8');
let sequence = 0;
async function harness(saved, { reduced = false, unsupported = false, blockedStorage = false } = {}) {
  ...
  globalThis.window={AudioContext:unsupported?undefined:Audio,matchMedia:()=>({matches:reduced})};
