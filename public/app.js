...
function assessmentScreen() {
  ...
  const cheers = ['¡Buen despegue!', '¡Una pista más en tu mapa!', '¡Conexión encontrada!', '¡Tu ruta sigue creciendo!'];
  shell(`<div class="assessment-wrap scene--${stage.key}">
    ...
    <progress class="journey-progress" max="10" value="${state.run.answered}" aria-label="Retos confirmados">${state.run.answered}/10</progress>
    ...
    <form id="answer-form">${options}
    ${feedback?'<div class="answer-feedback '+(feedback.correct?'success':'learning')+'" tabindex="-1" role="status"><strong>'+(feedback.correct?'✓ '+cheers[state.index%cheers.length]:'↗ Una pista para tu próximo intento')+'</strong><p>'+(feedback.correct?'Respuesta correcta.':'Respuesta incorrecta. La respuesta es <b lang="en">'+esc(feedback.correctAnswer)+'</b>.')+' '+esc(feedback.explanation)+'</p></div>':''}
...
document.addEventListener('submit', async e => {
  ...
  finally { state.busy=false; render(); focusScreen(formId==='answer-form' ? (state.message?'.notice':'.answer-feedback') : 'h1'); }
});
